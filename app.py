...
def fetch_alarm_config_from_db():
    """★ DB에서 온도 기준 정보를 가져옵니다 (저장 기능의 핵심)"""
    try:
        res = sb.table("room_settings").select("*").execute()
        if res.data:
            # DB 데이터를 딕셔너리로 변환 {'쌀창고': (5.0, 25.0), ...}
            config = {item['room_name']: (item['min_temp'], item['max_temp']) for item in res.data}
            # 없는 방은 기본값 채워주기
            for k, v in DEFAULT_ALARM_CONFIG.items():
...
with tabs[0]: # 대시보드
    ...
    if not raw_tasks: st.info("데이터가 없습니다.")
        ...
        filtered_df = df_all # (간략화: 전체 보기 기본)
...
with tabs[4]: # 조회/관리
    ...
    if tasks:
        ...
        st.dataframe(df[['issue_date', 'location', 'grade', 'issue_text', 'status']], use_container_width=True)
        
        st.divider()
        st.markdown("##### 🔧 상세 관리 (삭제/수정)")
        opts = [f"{t['issue_date']} | {t['location']} - {t['issue_text'][:15]}..." for t in tasks]
        sel_t = st.selectbox("관리할 항목 선택", opts)
        target = tasks[opts.index(sel_t)]
        
        c1, c2 = st.columns([3, 1])
        c1.warning(f"선택된 항목: {target['location']} - {target['issue_text']}")
        if c2.button("🗑️ 영구 삭제", type="primary"):
            delete_task_entirely(target['id'], target.get('photos'))
            st.success("삭제되었습니다."); st.rerun()
            
        with st.expander("등급/내용 수정"):
...
with tabs[5]:
    ...
    current_alarms = fetch_alarm_config_from_db() # ★ DB에서 불러옴
    
    # 2. 에디터용 데이터프레임 만들기
    if "alarm_df" not in st.session_state:
        data_list = []
        # DB에 있는 설정값으로 표를 만듭니다
