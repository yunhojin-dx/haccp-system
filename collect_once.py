...
# [2] 메인 로직
# =======================================================
print("🏭 [최종 수정판] 센서 수집 및 알림 로직 시작...")

try:
    if not API_KEY or not SUPABASE_URL: raise Exception("환경변수 오류")
    ...
    for sensor in SENSORS_BASE:
        ...
        if temp != -999:
            ...
            # ★ [핵심 수정] DB에 저장하기 '전'에 과거 기록부터 가져옵니다!
            prev_status = "정상" # 기록 없으면 정상으로 가정
            try:
            ...
            # ★ [비교] 과거(prev)와 현재(current)를 비교!
            if current_status == "비정상" and prev_status != "비정상":
                print(f"🔥 [{real_place_name}] 상태 변화 감지! (정상 -> 비정상) : 알림 예약")
                ...
                print(f"✅ [{real_place_name}] 상태 변화 감지! (비정상 -> 정상) : 알림 예약")
                alert_messages.append(f"✅ **{real_place_name} ({sensor['name']}) 온도 복구**\n> 현재: {temp}℃")
            else:
                print(f"🕊️ [{real_place_name}] 상태 변화 없음 (현재: {current_status} / 과거: {prev_status})")
